...
async function main() {
  ...
  console.log("Deploying Upgraded Vault with Clean RegimeUpdated Events");
  ...
  // 4. Wire Bridge Receiver
  console.log("4. Wiring Bridge Receiver on Vault...");
  const authBridgeReceiverTxHash = await walletClient.writeContract({
    address: vaultAddress,
    abi: artifacts.Vault.abi,
    functionName: "setBridgeReceiver",
    args: [bridgeReceiverAddress],
  });
  console.log("Vault setBridgeReceiver Tx Hash:", authBridgeReceiverTxHash);
  await publicClient.waitForTransactionReceipt({
    hash: authBridgeReceiverTxHash,
  });
  await sleep(2500);

  // 5. Pre-approve collateral tokens for new Vault
  console.log("5. Approving collateral tokens for new Vault...");
  ...
  // 6. Save updated deployment data
  const newDeploymentData = {
    ...
    vault: {
      ...
      bridgeAuthTx: authBridgeReceiverTxHash,
